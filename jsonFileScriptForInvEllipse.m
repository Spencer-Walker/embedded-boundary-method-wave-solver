...
for i=1:nx
    for j=1:ny        
        ...
        tempBool=get_bool_m_i_j_value_at_point_inv_ellipse( tmpPoint );
        ...
        if ( tempBool==0 )    % point is outside            
            ...
            elseif ( j==1 || j==ny )    % boundary point, default to outside
                ...
                lftPtBool=get_bool_m_i_j_value_at_point_inv_ellipse( leftPoint );
                
                rightPoint=[xVec(i+1) yVec(j)];
                rhtPtBool=get_bool_m_i_j_value_at_point_inv_ellipse( rightPoint );
                
                topPoint=[xVec(i) yVec(j+1)];
                tpPtBool=get_bool_m_i_j_value_at_point_inv_ellipse( topPoint );
                
                botPoint=[xVec(i) yVec(j-1)];
                btPtBool=get_bool_m_i_j_value_at_point_inv_ellipse( botPoint );
                ...
                if( lftPtBool==1 && rhtPtBool==1 && tpPtBool==1 && btPtBool==1 )
                    ...
                    if ( lftPtBool==0 )     % left point is outside
                        ...
                        LS_left  = get_bool_m_i_j_value_at_point_inv_ellipse( leftPoint );
                        LS_right = get_bool_m_i_j_value_at_point_inv_ellipse( tmpPoint );   % this is the current grid point                        
                    ...
                    elseif ( rhtPtBool==0 ) % right point is outside
                        ...
                        LS_left  = get_bool_m_i_j_value_at_point_inv_ellipse( tmpPoint );   % this is the current grid point                        
                        LS_right = get_bool_m_i_j_value_at_point_inv_ellipse( rightPoint );   
                        LS_bound = 0; % LS = 0 on boundary
                    ...
                    if ( tpPtBool==0 )     % top point is outside         
                        ...
                        LS_top = get_bool_m_i_j_value_at_point_inv_ellipse( topPoint );
                        LS_bot = get_bool_m_i_j_value_at_point_inv_ellipse( tmpPoint );   % this is the current grid point                        
                    ...
                    elseif ( btPtBool==0 ) % right point is outside
                        ...
                        LS_top = get_bool_m_i_j_value_at_point_inv_ellipse( tmpPoint ); % this is the current grid point                        
                        LS_bot = get_bool_m_i_j_value_at_point_inv_ellipse( botPoint );   
